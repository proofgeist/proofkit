...
export function makeMetadataCommand(): Command {
  ...
  metadata
    ...
    .option("--rich", "Include field metadata details", false)
    .action(async (opts, cmd) => {
      const globalOpts = cmd.parent?.parent?.opts() as ConnectionOptions & { pretty: boolean };
      try {
        const { db } = buildConnection(globalOpts);
        ...
        if (opts.rich) {
          const richFields = Object.fromEntries(
            fields.map(([fieldName, fieldMeta]) => {
              ...
              return [fieldName, { type: $Type, nullable: $Nullable, ...rest }];
            }),
          );
          printResult(richFields, { pretty: globalOpts.pretty ?? false });
          return;
        }
