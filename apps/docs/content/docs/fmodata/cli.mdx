...
```bash
fmodata query list --table contacts
fmodata query list --table contacts --top 10 --skip 20
fmodata query list --table contacts --select "name,email"
fmodata query list --table contacts --where "name eq 'Alice'"
fmodata query list --table contacts --order-by "name:asc"
fmodata query list --table contacts --order-by "createdAt:desc,name:asc"
```
...
```bash
fmodata query insert --table contacts --data '{"name":"Alice","email":"alice@example.com"}'
```
...
```bash
fmodata query delete --table contacts --where "status eq 'archived'"
```
...
```bash
fmodata query list --table contacts --top 5
```
...
server.tool(
  ...
  async ({ table, where, top }) => {
    const args = ["query", "list", "--table", table];
...
Use `fmodata query list --table <name> --top 1` to inspect a table's fields.
...
- **Read-only tasks**: restrict to `query list`, `metadata get/tables`
- **Automation tasks**: allow `query insert/update` and `script run`
