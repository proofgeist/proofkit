...
<Callout type="info" title="Hosted FileMaker is no longer required for scaffolding">
  ...
  When you are ready to generate layout clients, ProofKit prefers the local FM MCP path first.
...
- If FM MCP is running with a connected file, ProofKit can use that local path instead of a hosted server.
...
This command prefers a locally connected FM MCP file first, then falls back to `FM_SERVER` and `FM_DATABASE` from `.env`. It opens FileMaker Pro and runs the `Launch Web Viewer for Dev` script so your WebViewer points at the local dev server.
