...
<TypeTable
  type={{
    ...
    fmMcp: {
      ...
      description:
        "Optional local FM MCP mode for metadata fetching during typegen. Generated runtime clients still use the WebViewer adapter.",
    },
    generateClient: {
      type: "boolean",
      default: "true",
...
### `fmMcp`
If set, typegen uses the local FM MCP adapter only while generating schemas and clients. The generated runtime client still uses [@proofkit/webviewer](/docs/webviewer). If `webviewerScriptName` is not set, `fmMcp.scriptName` (or `"execute_data_api"`) is used for the generated webviewer client script name.
