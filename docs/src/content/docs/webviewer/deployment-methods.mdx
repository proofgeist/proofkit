...
There are many ways to deploy WebViewer code to your FileMaker file, and each method has its own pros and cons. This document attempts to outline the various options so you can make the best choice for your app.
...
- Updates require the web developer to have access to the FileMaker file

#### Considerations

If it weren’t for data migration challenges, the embedded method would clearly be the best choice. It aligns perfectly with our mental model for developing FileMaker solutions, where everything is contained within a single FileMaker file. If you're not dealing with data migrations, this is an excellent option. However, if you're working in a separate development environment—something we strongly recommend—you’ll need to be aware of the additional steps involved.
...
**Use a migration-only utility file.** If you need to keep your users in single open file in their FileMaker Pro client, or don't want to load the WebViewer code from a separate file, you can essentially create a mirror of the required WebViewer table in a utility file and use a [post-deployment script](https://docs.ottofms.com/concepts/deployments/deployment-scripts) to copy the code from the utility file to a table in the main file.

**Store the WebViewer code in a script** This method is generally not recommended because it's not possbile to automate the updating process whenever you make changes to the web code, but it can be used to store the code as **schema** in the file which will survive data migrations. This option may only work for simple widgets, as you might run into FileMaker's character limits.
...
This technique is a hybrid of the embedded and hosted methods. Essentially, the code is embedded in the FileMaker file, but you also host a copy of it on another web server. The FileMaker file (or the web code) can check for updates and download the latest version directly to the the necessary field.
