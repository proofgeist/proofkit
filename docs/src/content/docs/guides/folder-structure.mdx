...
A folder for your application's UI components. Components may also exist in the `app` directory if they are only used on a single page, but this folder is for components that are used across multiple pages.
